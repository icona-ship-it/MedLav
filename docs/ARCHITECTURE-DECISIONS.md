...
## ADR-026: Classificazione AI Act e obblighi di trasparenza (bozza da revisione legale)

- **Data**: 2026-09-05
- **Contesto**: ricerca online del 2026-09-05 (fonti secondarie legali, testi normativi non letti integralmente in Gazzetta): il regolamento "omnibus" (Reg. UE 2026/1744, GU 24/7/2026) sposta gli obblighi per i sistemi ad alto rischio dell'Allegato III al 2/12/2027, mentre l'art. 50 (trasparenza) e i divieti restano in vigore dal 2/8/2026, con periodo di grazia per la marcatura dei contenuti generati fino al 2/12/2026. La legge italiana 132/2025 (art. 13) impone al professionista di informare il cliente sull'uso di sistemi di IA.
- **Decisione (da confermare con un legale)**: (1) il modulo RC **stragiudiziale** di rc-mvp non rientra nell'Allegato III (nessuna amministrazione della giustizia: parere di parte, decisione umana piena); CTU/CTP, quando riattivati, vanno valutati contro l'Allegato III punto 8(a) e l'esenzione dell'art. 6(3)(d) prima del rilascio. (2) LegMed è "fornitore" ai fini dell'art. 50(2): gli export marcano il testo come prodotto con ausilio di IA e soggetto a revisione (proprietà del DOCX e `<meta name="generator">` degli HTML), entro il 2/12/2026. (3) Al perito viene fornito il testo-tipo dell'informativa art. 13 (`docs/INFORMATIVA-IA-ART13.md`). (4) La ZDR di Mistral non è dichiarata come fatto finché non confermata per iscritto; l'inferenza passa dall'endpoint regionale UE.
- **Conseguenze**: nessun obbligo di alto rischio pesa oggi sul prodotto; se cambiasse la classificazione (CTU/CTP) servono DPIA aggiornata, registrazione, sorveglianza umana documentata. Da fare: revisione legale di questo ADR, dell'informativa e dei Termini.

## ADR-027: Default "passaggi-chiave per rubrica" per la Documentazione Sanitaria dei nuovi casi RC

- **Data**: 2026-09-11
- **Contesto**: ADR-025 lasciava il default su 'selettiva' (LLM) "finché rubriche non batte il LLM anche sul caso semplice". Misure del panel: trascrizione LLM 67/47/37 (2026-09-04) contro rubriche 73/63/52 (giro 11, 2026-09-06): rubriche batte il LLM su tutti e tre i gold. In produzione però il selettore partiva da 'selettiva' e il valore non veniva salvato finché il perito non lo toccava: il medico che accettava i default riceveva la sezione NON misurata (audit 2026-09-10, reperto R3).
- **Decisione** (founder, 2026-09-11): il default per i nuovi casi RC è 'rubriche'. All'avvio dell'elaborazione (`/api/processing/start`) la modalità viene resa ESPLICITA in `perizia_metadata.docSanitariaMode` (`resolveDocSanitariaModeForStart`: la scelta del perito se valida, altrimenti il default; null per le pipeline senza sezione), così generazione, rigenerazione, viewer, export e link leggono lo stesso valore. Il selettore parte da 'rubriche'. I casi avviati prima, con metadati senza modalità, restano come sono (a lettura null continua a valere l'espansione precedente; a generazione null continua a valere 'selettiva'): nessun report esistente cambia.
- **Conseguenze**: il medico riceve la versione misurata dal gate; la conferma di Lavini sulla specifica (foglio 02) resta necessaria per le regole per tipo, non per il default. Da fare al rilascio: `PIPELINE_CHANGED_AT` (l'output dei nuovi casi cambia) e nota ai beta tester.

## ADR-028: Cronistoria «un fatto = una voce» con persistenza delle fusioni

- **Data**: 2026-09-21
- **Contesto**: il collaudo in produzione del 18/09 e i feedback dei medici (agosto) mostravano lo stesso accesso in Pronto Soccorso tre volte (verbale, certificato, referto successivo), il verbale di PS spezzato in cinque schede, tre foto di un referto lette come tre visite. La fusione fra documenti era disattivata da mesi perché il passo Inngest assumeva un rapporto 1:1 fra eventi consolidati e righe DB, e i doppioni scartati in memoria restavano visibili al medico.
- **Decisione**: (1) ogni evento consolidato porta l'identità della propria riga e l'elenco delle righe assorbite; il passo Inngest riscrive i sopravvissuti e poi elimina le righe assorbite (mai cancellare prima di scrivere; errore = retry). (2) Fusione fra documenti per «nucleo clinico» (stesso giorno, stessa famiglia, stessa procedura/distretto/lato): vince la fonte primaria sulla menzione, le fonti citate restano in nota. (3) Un accesso in PS è una voce (triage, consulenze, dimissione dentro; esami fuori); «ricovero» solo con esito di ammissione attestato. (4) Prestazioni senza importo non sono spese; la prognosi resta nel certificato; le prescrizioni date in visita stanno nella visita. (5) Mai fondere: lati opposti, orari diversi, diagnosi con sigle diverse, due fonti primarie con diagnosi diverse anche per una parola, medici diversi fra primarie, precisioni di data diverse, date assenti, spese. (6) I certificati medici a contenuto clinico restano nella vista clinica.
- **Conseguenze**: la cronistoria che il medico vede è quella della perizia; i casi già elaborati non cambiano finché non vengono rielaborati (`PIPELINE_CHANGED_AT`); il gate gold va rimisurato dopo il rilascio (giro 12). Residui dichiarati nel §11 di `scratchpad/analisi-10su10-2026-09-10.md`.
