...
| `0033_tiny_senator_kelly.sql` (merge multi-file documenti: 2 colonne + FK + indice parziale) | APPLICATA 2026-09-04 (`pnpm db:migrate` via pooler, autorizzata dal founder; verify 2/1/1) | si (indice parziale aggiunto a mano; niente RLS nuove: colonne su tabella già protetta) | `verify_0033.sql` |
| `0034_sad_guardsmen.sql` (events.temporal_scope: corrente/retrospettivo/programmato) | APPLICATA 2026-09-04 (stesso treno della 0033; verify 1/1/0; journal a 35 righe) | si (SQL riscritto idempotente: ADD COLUMN IF NOT EXISTS + CHECK in DO-block; niente RLS nuove) | `verify_0034.sql` |
| `0035_*` (future) | — | vedi procedura sotto | — |
