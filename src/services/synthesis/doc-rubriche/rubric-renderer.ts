...
const SIGNATURE_LINE_RE = /^(?:il medico (?:radiologo|refertante|specialista|chirurgo|di reparto)\b.*|il medico (?:curante|accettante|dimettente|di (?:ps|pronto soccorso|guardia|turno))\b\s*(?::|-|(?:dr|dott|prof)\.).*|(?:dr|dott|prof)\.?(?:\.?ssa)?\s+[\p{L}' .-]{2,40})$/iu;
